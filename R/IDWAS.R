...
#'This output will serve as the input for test(). 
#'@import org.Hs.eg.db
...
idwas<-function(drug_prediction, data, n=10, cnv){
  ...
  cols<-colnames(data)
  #_____________________________________________________________________________
  if ( (sum(grepl('TCGA', cols) == TRUE) > 0) | (length(grepl('TCGA', data$Tumor_Sample_Barcode)) > 0) ){ #If the patient samples are TCGA barcodes, then obtain the primary tumor/01A samples.
    if (cnv){
    ...
    if (cnv){
      ...
      if(!is.integer(dim)){ #If we have a vector (one gene)...
        ...
        suppressWarnings(for(i in 1:nrow(drug_prediction_filt_ord)){ #For each drug...
          pValList[[i]] <- numeric()
          betaValList[[i]] <- numeric()
          thecoefs <- coef(summary(lm(drug_prediction_filt_ord[i,]~commonlyMutated)))
          pValList[[i]]<- thecoefs[2,4]
          betaValList[[i]] <- thecoefs[2,1]
        })
